...
data_tmp <-data.frame(group=scRNA$seurat_clusters,
                      row.names = rownames(scRNA@meta.data))

for (i in 1:6) {
  tmp <- FeaturePlot(scRNA,features = gene_name[i])
  tmp <- tmp$data
  tmp <- data.frame(gene=as.numeric(tmp[,4]),
                    row.names = rownames(scRNA@meta.data))
...
new_vio <- reshape2::melt(data = data_tmp,
                          value.name = "exp")
new_vio$fill=ifelse(new_vio$group %in% c(0,1),"A","B")

ggplot(new_vio,aes(x = group,
                   y = exp)) +
...
source("stackvlion.R")
StackedVlnPlot(scRNA,features = gene_name,idents=c(0,1,2,3,4),
               cols=c("#7CCD7C","#7CCD7C",

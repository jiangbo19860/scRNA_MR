...
trait1 <- "ADHD"                      #性状1的名称，需要指定该参数

trait2 <- "epilepsy"                     #性状2的名称，需要指定该参数

trait1_samplesize <- 175226              #性状1的总样本量，需要指定该参数

trait2_samplesize <- 30470              #性状2的总样本量，需要指定该参数

#
# format_data_Catalog(GWASfile = "./1.data/GCST90568441_ADHD_modified.tsv",
#                     GWAS_name = trait1,
#                     type = "exposure",
...
#
#
# format_data_IEU_VCF(IEU_VCF = "./1.data/ieu-b-13_CAE.vcf.gz",
#                     type = "outcome",
#                     min_pval = 0,
...
TwosampleMR_dat_trait1 <- "ADHD_TwosampleMR.txt"

TwosampleMR_dat_trait2 <- "epilepsy_TwosampleMR.txt"

MTAG_dat_trait1 <- "ADHD_MTAG.txt"

MTAG_dat_trait2 <- "epilepsy_MTAG.txt"
...
nrow(data)  # 如果输出 0，说明合并失败！
head(data)  # 查看合并后是否有SNP和p_SHet列

sig_ind_snp <- clump_data_local_Online(data,
                                       snp_col = "SNP",
                                       pval_col = "p_SHet",
                                       clump_pval = 5e-8,  # 关键修改：设置为你的筛选阈值
                                       clump_kb = 500,
                                       clump_r2 = 0.3,
...
data <- as.data.frame(data.table::fread("./2.pre_data/ADHD_SMR_Catalog.ma"))

data <- subset(data,data$p<5e-8)

data.table::fwrite(x = as.data.frame(data), file = "./2.pre_data/ADHD_SMR_Catalog_sig.ma",
...
# 分别将ADHD_SMR_Catalog_sig.ma与ukb-b-3957_SMR_IEU_sig.ma上传到SMR在线网站，对共同显著的组织进行SMR分析

...
p_full = ggboxplot(data_melt_full, x = "Gene", y = "Expression", color = "Type",
                   xlab = "", ylab = "Gene expression (all genes)",  # 标题明确标注“所有基因”
                   legend.title = "Type",
                   palette = c("green", "purple"),  # 用不同颜色区分于显著基因图
                   add = "point",
                   width = 0.8)
p_full = p_full + rotate_x_text(60)  # 旋转x轴标签
...
# 保存所有基因的箱线图（文件名标注“all_genes”）
pdf(file = "boxplot_all_genes.pdf", width = 12, height = 6)  # 适当增大画布以容纳更多基因
print(p_full)
dev.off()
...
# 5. 原始数据结果输出（文件+可视化）
# --------------------------
today_date <- format(Sys.Date(), "%Y%m%d")  # 日期标签
...
if (sig_count > 0) {
  ...
  print(p)
  dev.off()
  cat("原始箱线图已保存至：", boxplot_filename, "\n")
} else {
...
write.table(reanalysis_results,
            file = here("3_outputs", paste0(today_date, "_reanalysis_without_zero_samples.txt")),
            sep = "\t", quote = FALSE, row.names = FALSE)
cat("\n重新分析结果已保存至：", paste0(today_date, "_reanalysis_without_zero_samples.txt"), "\n")

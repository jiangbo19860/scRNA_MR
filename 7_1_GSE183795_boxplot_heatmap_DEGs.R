# 对基因表达数据进行差异分析，并通过热图和箱线图展示显著差异表达的基因在不同样本组（对照组 Control 和肿瘤组 Tumor）中的表达模式。
# 1个输入：差异基因表达数据3_outputs/20250804_snpExp_37genes_32samples.txt。输出有3个，2个图和一个txt文件。
rm(list = ls())  # 清除工作空间
...
# --------------------------
# 读取表达数据
rt <- read.table(here("3_outputs/20250810_snpExp_38genes_244samples.txt"),
...
# --------------------------
# 2. 提取分组信息并修正（核心修正：同步过滤样本）
# --------------------------
Type <- gsub("(.*)\\_(.*)", "\\2", colnames(data))  # 从样本名提取分组

# 查看原始分组水平（确认问题）
cat("修正前的分组水平：", unique(Type), "\n")
cat("修正前的样本数：", table(Type), "\n")

# 剔除"name"分组的样本
valid_samples <- Type %in% c("Control", "Tumor")  # 仅保留Control和Tumor
data <- data[, valid_samples, drop = FALSE]  # 过滤表达矩阵
Type <- Type[valid_samples]  # 更新分组变量
exp_full <- exp_full[, valid_samples, drop = FALSE]  # 同步过滤完整数据集（关键修正）

# 再次检查分组水平（确保只有2个）
cat("修正后的分组水平：", unique(Type), "\n")
cat("修正后的样本数：", table(Type), "\n")
...
# --------------------------
# 新增：绘制所有基因的箱线图（修正后：样本数与分组匹配）
# --------------------------
# 转换过滤后的完整数据格式（使用同步过滤的exp_full）
exp_full_t <- as.data.frame(t(exp_full))  # 转置过滤后的完整数据（样本数与Type一致）
exp_full_t <- cbind(exp_full_t, Type = Type)  # 添加分组信息（此时行数匹配）
...
# 计算基因总数（行名为基因名，统计行数即为基因数量）
gene_count <- nrow(exp_full)

# 保存所有基因的箱线图（文件名中添加基因数量）
today_date <- format(Sys.Date(), "%Y%m%d")  # 统一日期变量
all_genes_boxplot <- paste0(today_date, "_boxplot_all_", gene_count, "genes.pdf")  # 嵌入基因数量
...
# 6.3 提取原始p值（修正版：使用过滤后的data而非original_data）
for (gene in problem_genes) {
  # 使用过滤后的data（与Type样本数一致）
  test_original <- wilcox.test(data[gene, ] ~ Type, exact = FALSE)

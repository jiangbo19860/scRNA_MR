...
cat("\n结果已保存至:", output_file, "\n")
# 自动同步指定目录下的.R文件到Git仓库
# 功能：检测变更 → 提交本地 → 推送远程 → 记录日志

# ----------------------
# 配置参数（根据实际情况修改）
# ----------------------
target_dir <- "/Users/lijiangbo/scRNA_MR/2_src"  # 要同步的目录（含.R文件）
remote_repo <- "origin"                          # 远程仓库别名（通常为origin）
branch <- "main"                                 # 要推送的分支
log_file <- "/Users/lijiangbo/git_sync_log.txt"  # 日志文件路径
...
log_message <- function(text) {
  ...
  log_entry <- paste0("[", timestamp, "] ", text, "\n")
  cat(log_entry, file = log_file, append = TRUE)
  cat(log_entry)  # 同时在控制台输出
}
...
tryCatch({
  ...
  if (!dir.exists(target_dir)) {
    stop("目标目录不存在: ", target_dir)
  }
  setwd(target_dir)  # 切换到目标目录

  # 检查是否为Git仓库
  if (system("git rev-parse --is-inside-work-tree", intern = TRUE) != "true") {
    stop("目标目录不是Git仓库，请先初始化")
  }

  log_message("开始执行同步任务...")
  ...
  if (length(r_files) == 0) {
    log_message("未发现.R文件，同步终止")
    quit(status = 0)
  }

  # 检查是否有变更（比较工作区与暂存区）
  status_output <- system("git status --porcelain", intern = TRUE)
  changed_files <- grep(paste(r_files, collapse = "|"), status_output, value = TRUE)

  if (length(changed_files) == 0) {
    log_message("没有需要同步的.R文件变更")
    quit(status = 0)
  }
  ...
  # 提交变更到本地仓库
  # ----------------------
  log_message("发现以下变更文件:")
  log_message(paste("  -", changed_files, collapse = "\n"))

  # 添加所有变更的.R文件
  add_result <- system(paste("git add", paste(r_files, collapse = " ")), intern = TRUE)

  # 生成提交信息（日期 + 变更文件数）
  today <- format(Sys.Date(), "%Y-%m-%d")
  commit_msg <- paste0("Auto-sync on ", today, " (", length(changed_files), " files changed)")
  commit_result <- system(paste0("git commit -m '", commit_msg, "'"), intern = TRUE)

  if (any(grepl("nothing to commit", commit_result))) {
    log_message("所有变更已提交，无需新提交")
  } else {
    log_message("成功提交到本地仓库: ", commit_msg)
  }

  # ----------------------
  # 推送到远程仓库
  # ----------------------
  push_cmd <- paste("git push", remote_repo, branch)
  push_result <- system(push_cmd, intern = TRUE)

  if (any(grepl("Everything up-to-date", push_result)) ||
      any(grepl("To git@github.com", push_result))) {
    log_message("成功推送到远程仓库: ", remote_repo, "/", branch)
  } else {
    stop("推送失败: ", paste(push_result, collapse = "; "))
  }
  ...
  # 捕获并记录错误信息
  log_message(paste("同步失败: ", e$message))
  quit(status = 1)
})
# 自动同步指定目录下的.R文件到Git仓库
# 功能：检测变更 → 提交本地 → 推送远程 → 记录日志
